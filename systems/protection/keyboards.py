...
def settings_keyboard(settings: dict) -> InlineKeyboardMarkup:
    ...
    bots_icon = "✅" if settings.get("block_bots", True) else "❌"
    buttons.append([
        InlineKeyboardButton(text=f"{bots_icon} 🤖 منع البوتات", callback_data="owner:prot_toggle:block_bots")
    ])

    buttons.append([InlineKeyboardButton(text="📝 الكلمات المحظورة", callback_data="owner:prot_words:0")])
    buttons.append([InlineKeyboardButton(text="⚙️ خيارات النظام", callback_data="owner:prot_sys_words:0")])
...
def system_words_keyboard(words: list[str], offset: int = 0, limit: int = 10) -> InlineKeyboardMarkup:
    """قائمة خيارات النظام (قائمة منفصلة عن الكلمات المحظورة العادية) مع 🗑️/➕."""
    buttons = []
    ...
    for word in page_words:
        buttons.append([
            InlineKeyboardButton(text=f"⚙️ {word}", callback_data="owner:noop"),
            InlineKeyboardButton(text="🗑️", callback_data=f"owner:prot_sys_word_remove:{word}"),
        ])

    nav_row = []

    if offset > 0:
        nav_row.append(InlineKeyboardButton(text="◀️ رجوع", callback_data=f"owner:prot_sys_words:{offset - limit}"))

    if offset + limit < len(words):
        nav_row.append(InlineKeyboardButton(text="التالي ▶️", callback_data=f"owner:prot_sys_words:{offset + limit}"))

    if nav_row:
        buttons.append(nav_row)

    buttons.append([InlineKeyboardButton(text="➕ إضافة", callback_data="owner:prot_sys_word_add")])
    buttons.append([InlineKeyboardButton(text="🔙 رجوع", callback_data="owner:protection")])

    return InlineKeyboardMarkup(inline_keyboard=buttons)


def system_words_cancel_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="❌ إلغاء", callback_data="owner:prot_sys_words:0")],
        ]
    )
