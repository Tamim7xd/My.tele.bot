...
محتوى الإعلان، ويحذفه تلقائياً بعد المدة المحددة (إن وُجدت).

⚠️ ملاحظة عن الترتيب وآلية التمرير:
هذا الـ router يستخدم F.text بدون قائمة ثابتة (الأوامر ديناميكية
من قاعدة البيانات)، لذا lookup يحدث لكل رسالة نصية قصيرة. عند عدم
وجود إعلان مطابق، يجب تمرير الرسالة للأنظمة التالية (members) عبر
رفع SkipHandler - وإلا ستُبتلع كل الرسائل القصيرة (مثل "حساب") هنا.
يجب أن يُسجَّل قبل members_system.router (الذي يطابق كل الرسائل).
"""
