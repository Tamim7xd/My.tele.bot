...
async def set_member_membership(pool: asyncpg.Pool, user_id: int, membership_id: str, duration_seconds: int) -> datetime:
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            "UPDATE members SET membership_id = $1, membership_expires_at = $2, membership_expiry_warned = FALSE WHERE user_id = $3",
...
async def extend_membership(pool: asyncpg.Pool, user_id: int, extra_seconds: int) -> datetime | None:
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            "UPDATE members SET membership_expires_at = $1, membership_expiry_warned = FALSE WHERE user_id = $2",
            new_expires, user_id,
        )
...
# ===== تنبيه انتهاء العضوية قبل يوم =====

async def get_members_expiring_soon(pool: asyncpg.Pool, within_hours: int = 24) -> list[asyncpg.Record]:
    """
    يرجع كل الأعضاء الذين تنتهي عضويتهم خلال within_hours ساعة القادمة،
    ولم يُرسَل لهم تنبيه الانتهاء بعد (membership_expiry_warned = FALSE).
    العضويات الدائمة (membership_expires_at IS NULL) لا تُحسب.
    """
    async with pool.acquire() as conn:
        return await conn.fetch(
            """
            SELECT user_id, username, full_name, membership_id, membership_expires_at
            FROM members
            WHERE membership_id IS NOT NULL
              AND membership_expires_at IS NOT NULL
              AND membership_expires_at > NOW()
              AND membership_expires_at <= NOW() + ($1 || ' hours')::interval
              AND membership_expiry_warned = FALSE
            """,
            str(within_hours),
        )


async def mark_expiry_warned(pool: asyncpg.Pool, user_id: int) -> None:
    async with pool.acquire() as conn:
        await conn.execute(
            "UPDATE members SET membership_expiry_warned = TRUE WHERE user_id = $1",
            user_id,
        )


async def reset_expiry_warned(pool: asyncpg.Pool, user_id: int) -> None:
    """يُعاد ضبطه عند منح/تجديد/تمديد عضوية، حتى يُرسَل تنبيه جديد عند الاقتراب من الانتهاء التالي."""
    async with pool.acquire() as conn:
        await conn.execute(
            "UPDATE members SET membership_expiry_warned = FALSE WHERE user_id = $1",
            user_id,
        )
