...
# ===== خيارات النظام (قائمة منفصلة) =====

@router.callback_query(F.data.startswith("owner:prot_sys_words:"))
async def show_system_words(callback: CallbackQuery, state: FSMContext) -> None:
    if callback.message is None or not _is_owner(callback.from_user.id):
        await callback.answer()
        return
    ...
    pool = await get_pool()
    words = await protection_queries.get_system_words(pool)

    text = messages.system_words_text(words)
    keyboard = prot_keyboards.system_words_keyboard(words, offset)

    await callback.message.edit_text(text, reply_markup=keyboard)
    await callback.answer()


@router.callback_query(F.data.startswith("owner:prot_sys_word_remove:"))
async def remove_system_word(callback: CallbackQuery) -> None:
    if callback.message is None or not _is_owner(callback.from_user.id):
        await callback.answer()
        return
    ...
    pool = await get_pool()
    words = await protection_queries.remove_system_word(pool, word)

    text = messages.system_words_text(words)
    keyboard = prot_keyboards.system_words_keyboard(words, 0)

    await callback.message.edit_text(text, reply_markup=keyboard)
    await callback.answer(messages.system_word_removed_text(word))


@router.callback_query(F.data == "owner:prot_sys_word_add")
async def add_system_word_prompt(callback: CallbackQuery, state: FSMContext) -> None:
    if callback.message is None or not _is_owner(callback.from_user.id):
        await callback.answer()
        return

    await state.set_state(OwnerStates.waiting_protection_system_word)

    await callback.message.edit_text(
        messages.ADD_SYSTEM_WORD_PROMPT,
        reply_markup=prot_keyboards.system_words_cancel_keyboard(),
    )
    await callback.answer()


@router.message(OwnerStates.waiting_protection_system_word)
async def add_system_word_receive(message: Message, state: FSMContext) -> None:
    if not _is_owner(message.from_user.id if message.from_user else None):
        return
    ...
    pool = await get_pool()
    existing_words = await protection_queries.get_system_words(pool)

    if word in existing_words:
        await message.reply(
            messages.ADD_SYSTEM_WORD_DUPLICATE,
            reply_markup=prot_keyboards.system_words_cancel_keyboard(),
        )
        return

    words = await protection_queries.add_system_word(pool, word)
    await state.clear()

    text = messages.system_words_text(words)
    keyboard = prot_keyboards.system_words_keyboard(words, 0)

    await message.reply(messages.system_word_added_text(word), reply_markup=keyboard)
