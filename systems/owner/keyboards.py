...
def member_page_keyboard(user_id: int, offset: int) -> InlineKeyboardMarkup:
    """
    صفحة عضو كاملة من قائمة الأعضاء العامة:
    تعديل الرصيد + رجوع لقائمة الأعضاء.
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🔙 رجوع", callback_data=f"owner:members:{offset}")],
        ]
    )
